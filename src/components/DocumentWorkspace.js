...
import {SHARED_LEAD_IN_IDS,defineAdvanceState,nextDmaicArtifact,phaseTollgateRoute,previousDmaicArtifact,projectDocumentRoute} from '../utils/defineSequence';
import {cadenceForPhase,phaseHomeRoute} from '../config/oeProfessionalCadence';
...
import WorkflowAnalysisConnection from './WorkflowAnalysisConnection';
...
export default function DocumentWorkspace({ template, project, updateProject, standalone=false, onGuidedState }) {
  ...
  const { datasets,activeDataset } = useWorksheet();
  ...
  const phaseCadence=cadenceForPhase(template.phase);
  const cadenceStep=[...phaseCadence.core,...phaseCadence.conditional,...phaseCadence.additional].find(step=>step.id===template.id);
  const isCoreCadenceStep=phaseCadence.core.some(step=>step.id===template.id);
  const phaseBoundary=lifecycle.id==='operational-excellence'&&isCoreCadenceStep&&cadenceStep?.sequence!==false&&!sequenceNext;
  const optionalCadenceStep=lifecycle.id==='operational-excellence'&&(!isCoreCadenceStep||cadenceStep?.sequence===false)&&template.phase!=='Define';
  const relevantAnalysisConnections=useMemo(()=>{
    const all=phaseCadence.additional.filter(item=>item.kind==='analysis');
    const idsByDocument={
      'msa-workspace':['msa'],'process-capability-study':['capability'],'baseline-metrics':['descriptive','histogram','capability'],
      'hypothesis-plan':['hypothesis','anova','regression','multiregression','logistic','correlation','distribution'],
      'fishbone-workspace':['fishbone-tool','pareto'],'root-cause-verification':['hypothesis','anova','regression','multiregression','logistic','correlation','distribution','pareto'],
      'statistical-analysis-summary':['hypothesis','anova','regression','multiregression','logistic','correlation','distribution','pareto'],
      'factorial-plan':['doe'],'solution-selection-matrix':['fmea'],'impact-effort-matrix':['fmea'],'pilot-plan':['validation'],'implementation-plan':['fmea','validation'],'action-plan':['validation'],
      'control-plan':['control-chart','attribute-chart','run-chart'],'reaction-plan':['control-chart','attribute-chart','run-chart'],'monitoring-plan':['control-chart','attribute-chart','run-chart','capability-revalidation']
    };
    const ids=idsByDocument[template.id]||[];
    return all.filter(item=>ids.includes(item.id));
  },[phaseCadence.additional,template.id]);
  const toggleAnalysisReference=id=>setRecord(previous=>{const ids=previous.references?.analysisIds||[];return{...previous,references:{...previous.references,analysisIds:ids.includes(id)?ids.filter(item=>item!==id):[...ids,id]}}});
  const openSequenceArtifact=async artifact=>{if(!artifact||!project.id)return;const saved=await saveNow();if(!saved)return;if(artifact.kind==='action'&&artifact.route==='datasets'){navigate(`/projects/${project.id}?tab=datasets&phase=${encodeURIComponent(template.phase)}`);return;}navigate(artifact.id==='charter'?`/projects/${project.id}/charter`:projectDocumentRoute(project.id,artifact.id));};
  const openSuccessor=async()=>{if(!project.id){setNotice('A valid project is required before opening the next document.');return;}if(phaseBoundary){const saved=await saveNow();if(saved)navigate(phaseTollgateRoute(project.id,template.phase));return;}if(optionalCadenceStep){const saved=await saveNow();if(saved)navigate(phaseHomeRoute(project.id,template.phase));return;}await openSequenceArtifact(sequenceNext);};
  const requestSequenceNext=()=>{if(!sequenceNext&&!phaseBoundary&&!optionalCadenceStep)return;if(advanceState.missing.length){setProgressionWarning(true);return;}openSuccessor();};
  const guided=Boolean(location.state?.guided);
  const advance=async()=>{if(!advanceState.atLast){const saved=await saveNow();if(saved){setActiveIndex(index=>index+1);setNotice('Section saved · continue to the next section');}return;}if(advanceState.next||phaseBoundary||optionalCadenceStep){if(advanceState.missing.length){setProgressionWarning(true);return;}await openSuccessor();return;}setNotice(`${template.name} completes the configured workflow. Return to the project workspace when ready.`);};
