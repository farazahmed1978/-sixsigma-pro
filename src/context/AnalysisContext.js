...
export const shouldPersistAnalysis = analysis => analysis?.provenanceStatus !== 'legacy-incomplete';
...
export function AnalysisProvider({ children }) {
  ...
  useEffect(()=>{let active=true;if(!configured||!user||!profile?.default_organization_id)return()=>{active=false};const request=nextGeneration(generation.current);generation.current=request;setAnalysisResults([]);setHydrationStatus(HYDRATION.HYDRATING);setPersistenceError('');analysisRepository.listOrganization(profile.default_organization_id).then(rows=>{if(active&&isCurrentGeneration(request,generation.current)){setAnalysisResults(rows.map(row=>({...row.content,id:row.id,projectId:row.project_id,organizationId:row.organization_id,createdBy:row.created_by,title:row.title,status:row.status,phase:row.dmaic_phase,datasetVersionIds:row.dataset_version_ids||row.content?.datasetVersionIds||[],variableMapping:row.variable_mapping||row.content?.variableMapping||{},provenanceStatus:row.provenance_status||'legacy-incomplete',executedAt:row.executed_at||row.content?.executedAt||row.created_at,createdAt:row.created_at,updatedAt:row.updated_at})));setHydrationStatus(HYDRATION.READY)} }).catch(error=>{if(active&&isCurrentGeneration(request,generation.current)){setHydrationStatus(HYDRATION.ERROR);setPersistenceError(error.message||'Analyses could not be loaded from Aureqin.')}});return()=>{active=false}},[configured,profile?.default_organization_id,user]);
  useEffect(() => {if(configured){if(!user||!profile?.default_organization_id)return;const timer=window.setTimeout(()=>Promise.all(analysisResults.filter(item=>item.projectId&&shouldPersistAnalysis(item)).map(item=>analysisRepository.save({id:item.id,project_id:item.projectId,organization_id:item.organizationId||profile.default_organization_id,created_by:item.createdBy||user.id,status:item.status||'complete',methodology:item.methodology||'lean-six-sigma',lifecycle_phase:item.phase||'Analyze',dmaic_phase:item.phase||'Analyze',title:item.title||item.name||'Analysis',method:item.method||item.toolType||'',method_version:item.methodVersion||'1',dataset_version_ids:item.datasetVersionIds||[],parameters:item.inputConfiguration||{},variable_mapping:item.variableMapping||{},result_schema_version:item.resultSchemaVersion||'1',validation_status:item.validationStatus||'UNVALIDATED',provenance_status:item.provenanceStatus||'complete',executed_at:item.executedAt||item.createdAt||new Date().toISOString(),content:item,updated_at:new Date().toISOString()}))).catch(error=>console.error('Analyses could not be saved to Aureqin:',error)),250);return()=>window.clearTimeout(timer)}try { localStorage.setItem(STORAGE_KEY, JSON.stringify(analysisResults)); } catch (error) { console.warn('Analyses could not be saved:', error); } }, [analysisResults,configured,profile?.default_organization_id,user]);
