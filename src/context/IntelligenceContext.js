...
const validFavorite=item=>item&&typeof item==='object'&&typeof item.type==='string'&&item.type&&typeof item.id==='string'&&item.id&&typeof item.path==='string'&&item.path.startsWith('/');
const loadFavorites=()=>{try{const parsed=JSON.parse(localStorage.getItem(FAVORITES_KEY)||'[]');return Array.isArray(parsed)?parsed.filter(validFavorite):[]}catch{return[]}};
...
export function IntelligenceProvider({children}){
 ...
 const toggleFavorite=useCallback(asset=>{if(!validFavorite(asset))return;setFavorites(previous=>previous.filter(validFavorite).some(item=>item.type===asset.type&&item.id===asset.id)?previous.filter(item=>validFavorite(item)&&!(item.type===asset.type&&item.id===asset.id)):[asset,...previous.filter(validFavorite)])},[]);
 const isFavorite=useCallback((type,id)=>favorites.some(item=>validFavorite(item)&&item.type===type&&item.id===id),[favorites]);
 const index=useMemo(()=>[
