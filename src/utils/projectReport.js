...
// sequenceForProject already resolves the OE-vs-PM stage ordering without branching here (Define →
// Measure → Analyze → Improve → Control, or Initiation → Planning → Execution → Monitoring &
// Controlling → Closing); this just filters that sequence down to templates with a
// project.documents entry, so a document a user never opened never appears as an empty placeholder
// in the combined report — and resolves each one's display phase and diagram the same way
// DocumentWorkspace does for the per-document Print button, so a document never renders
...
export function projectReportEntries(project){
  ...
  return sequenceForProject(project).map(template=>{
