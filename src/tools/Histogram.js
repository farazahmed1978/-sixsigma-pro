...
export default function DescriptiveStats() {
  ...
  const analyze = (colOverride) => {
    const useCol = colOverride !== undefined ? colOverride : col;
    setAddedToReport(false);
    let data = useCol && hasData ? getColumnData(useCol) : manualData.split(/[\n,\s]+/).map(parseFloat).filter(v => !isNaN(v));
  ...
  // If the guided flow (GuidedHome) passed a pre-selected metric variable, load and run it automatically.
  useEffect(() => {
    const preselected = location.state?.selectedMetric?.[0];
    if (preselected && hasData && numCols.some(c => c.name === preselected)) {
      setCol(preselected);
      analyze(preselected);
    }
