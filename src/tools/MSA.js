...
export default function MSA() {
  ...
  const addToReport = async () => { const analysisId = persistAnalysis(); await addReportItem({ title: `${currentStudy[1]} — ${mapping.measurement || mapping.rating}`, toolId: 'msa', projectId: activeDataset?.projectId, phase: 'Measure', timestamp: new Date().toISOString(), statsSummary: Object.fromEntries(summary.metrics), interpretation: summary.status, structuredOutput: structuredReportPayload({ method: result.method, methodVersion: result.methodVersion, configuration: { studyType, mapping, tolerance }, metrics: result, tables: { anova: result.anova || [], withinAppraiser: result.withinAppraiser || [], betweenAppraiser: result.betweenAppraiser || [] }, plots: studyType === 'stability' ? { controlChart: result.chart } : {}, signals: result.chart?.violations || [], interpretation: summary.status }), provenance: { analysisId, datasetId: activeDataset?.id, datasetVersionId: activeDataset?.versionId, validationStatus: MSA_VALIDATION_STATUS[studyType] } }); setNotice('Structured MSA result added to the report binder.'); };
