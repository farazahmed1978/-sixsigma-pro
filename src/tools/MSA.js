...
import { Bar, BarChart, Legend, LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ReferenceLine, ResponsiveContainer } from 'recharts';
...
export default function MSA() {
  ...
  const gageDiagnostics = useMemo(() => {
    if (!result || !['crossed', 'nested'].includes(studyType)) return null;
    const measurements = raw(mapping.measurement), parts = raw(mapping.part), operators = raw(mapping.operator);
    const rows = measurements.map((value, index) => ({ part: String(parts[index]), operator: String(operators[index]), value: Number(value) })).filter(row => row.part && row.operator && Number.isFinite(row.value));
    const operatorNames = [...new Set(rows.map(row => row.operator))];
    const operatorMeans = operatorNames.map(operator => { const values = rows.filter(row => row.operator === operator).map(row => row.value); return { operator, mean: values.reduce((sum, value) => sum + value, 0) / values.length }; });
    const partProfiles = [...new Set(rows.map(row => row.part))].map(part => Object.assign({ part }, ...operatorNames.map(operator => { const values = rows.filter(row => row.part === part && row.operator === operator).map(row => row.value); return { [operator]: values.length ? values.reduce((sum, value) => sum + value, 0) / values.length : null }; })));
    const components = ['repeatability', 'reproducibility', 'gageRR', 'partToPart'].filter(component => result.percentStudyVariation[component] !== undefined).map(component => ({ component, percentStudyVariation: result.percentStudyVariation[component] }));
    return { operatorNames, operatorMeans, partProfiles, components };
  // The active worksheet/upload rows are immutable for a completed result; rerun after remapping data.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [result, studyType, mapping.measurement, mapping.part, mapping.operator]);
  ...
  const createFindingAction = () => { if (summary.tone === 'success') { setNotice('No evidence-supported MSA finding condition was detected; no finding was created.'); return; } const analysisId = persistAnalysis(); const finding = createPractitionerFinding({ analysisId, dataset: activeDataset, variable: mapping.measurement || mapping.rating, type: 'measurement_system_inadequate', severity: summary.tone === 'danger' ? 'critical' : 'warning', statement: `${currentStudy[1]}: ${summary.status}.`, evidence: { resultPath: 'result', studyType, rule: 'Practitioner assessment is warning or danger', metrics: summary.metrics } }); localStorage.setItem('aureqin_latest_finding', JSON.stringify(finding)); setNotice('Finding created with dataset-version and analysis provenance.'); };
  ...
  return <div className="practitioner-workflow">
    ...
    {result && summary && <section className="workflow-card results-card"><div className={`assessment-banner ${summary.tone}`}><span>Measurement system assessment</span><strong>{summary.status}</strong></div><div className="stat-grid">{summary.metrics.map(([label, value]) => <div className="stat-card" key={label}><div className="stat-value">{value}</div><div className="stat-label">{label}</div></div>)}</div>
      ...
      {(studyType === 'crossed' || studyType === 'nested') && <><h3>Variation hierarchy</h3><div className="table-scroll"><table className="professional-table"><thead><tr><th>Component</th><th>Variance component</th><th>% Contribution</th><th>% Study variation</th><th>% Tolerance</th></tr></thead><tbody>{Object.keys(result.varianceComponents).map(component => <tr key={component}><td>{component}</td><td>{formatMetric(result.varianceComponents[component])}</td><td>{formatMetric(result.percentContribution[component],1)}%</td><td>{result.percentStudyVariation[component]===undefined?'—':`${formatMetric(result.percentStudyVariation[component],1)}%`}</td><td>{result.percentTolerance?.[component]===undefined?'—':`${formatMetric(result.percentTolerance[component],1)}%`}</td></tr>)}</tbody></table></div><div className="definition-callout"><strong>Hierarchy:</strong> Total Gage R&R combines repeatability and reproducibility. Reproducibility includes operator/appraiser variation and, for crossed studies, part × appraiser interaction. ndc = {result.ndc ?? 'not estimable'}.</div></>}
      {gageDiagnostics&&<><h3>Operator comparison</h3><ResponsiveContainer width="100%" height={240}><BarChart data={gageDiagnostics.operatorMeans}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="operator"/><YAxis/><Tooltip/><Bar dataKey="mean" fill="var(--accent)"/></BarChart></ResponsiveContainer><h3>Part × appraiser interaction</h3><ResponsiveContainer width="100%" height={260}><LineChart data={gageDiagnostics.partProfiles}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="part"/><YAxis/><Tooltip/><Legend/>{gageDiagnostics.operatorNames.map((operator,index)=><Line key={operator} dataKey={operator} stroke={index%2?'var(--red)':'var(--accent)'} connectNulls={false} dot/>)}</LineChart></ResponsiveContainer><h3>Repeatability and reproducibility</h3><ResponsiveContainer width="100%" height={230}><BarChart data={gageDiagnostics.components}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="component"/><YAxis/><Tooltip/><Bar dataKey="percentStudyVariation" fill="var(--accent)"/></BarChart></ResponsiveContainer></>}
