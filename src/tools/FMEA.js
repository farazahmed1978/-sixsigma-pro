...
import { interpretFMEA } from '../utils/interpretations';
import './Tool.css';
...
export default function FMEA() {
  ...
  const handleAddToReport = useCallback(async () => {
    if (!tableWrapperRef.current || !rows.length) return;

    const canvas = await html2canvas(tableWrapperRef.current, { backgroundColor: null, scale: 2 });
    const chartImage = canvas.toDataURL('image/png');

    const interpretation = interpretFMEA({ rows });
    ...
    addReportItem({
      ...
      statsSummary: {
        ...
        'Critical (≥200)': critCount,
        'High (100–199)': highCount,
        'Avg RPN': avgRPN,
      },
      interpretation,
      rawData: rows.map(r => ({
        processStep: r.processStep, failureMode: r.failureMode, effect: r.effect,
        severity: r.severity, occurrence: r.occurrence, detection: r.detection,
        rpn: r.rpn, priority: getRPNLabel(r.rpn), action: r.action, status: r.status,
      })),
    });

    setAddedToReport(true);
    setTimeout(() => setAddedToReport(false), 2500);
  }, [rows, critCount, highCount, avgRPN, addReportItem]);
