...
export const charterSaveStateLabel=state=>({saving:'Saving…',saved:'Saved',unsaved:'Unsaved changes'}[state]||'Unsaved changes');
...
function Preview({ project, charter, completion, quality, onClose, previewRef, phaseLabel }) {
...
export default function ProjectCharter() {
  ...
  // DocumentWorkspace.js already uses, not a hardcoded SIPOC hop — otherwise a PM project's Next
  // incorrectly lands on an OE-only document.
  ...
  return <WorkspaceShell className="project-charter-shell" mode={workspaceMode} backTo={`/projects/${id}`} backLabel="Project" breadcrumb={<><Link to={`/projects/${id}`}>Project Home</Link><span> / {currentStageLabel} / Project Charter</span></>} previousLabel="Previous" previousDisabled={false} onPrevious={() => navigate(-1)} sequencePreviousLabel="Previous item" sequencePreviousDisabled sequenceNextLabel={nextDoc?.sequenceLabel} sequenceNextDisabled={!nextDoc} onSequencePrevious={()=>{}} onSequenceNext={requestNextDocument} onMinimize={() => setWorkspaceMode('minimized')} onMaximize={() => setWorkspaceMode('maximized')} onRestore={() => setWorkspaceMode('normal')} onSave={saveNow} saving={saveState==='saving'} onExport={exportPdf} onPrint={() => window.print()} onHelp={() => setGuideOpen(true)} suiteId={lifecycle.id}><div className="pc-page">
    ...
    {progressionWarning&&<div className="pc-progression-modal" role="dialog" aria-modal="true" aria-labelledby="pc-progression-title"><button type="button" className="pc-progression-backdrop" onClick={()=>setProgressionWarning(false)} aria-label="Stay here"/><section><span>GUIDED PROGRESSION</span><h2 id="pc-progression-title">This document is incomplete.</h2><p><strong>{completion}% complete</strong> · {missingRequiredCount} required {missingRequiredCount===1?'item remains':'items remain'}.</p><ul>{missingSections.slice(0,6).map(section=><li key={section.id}>{section.label}</li>)}</ul><p>You can continue to {nextDoc?.sequenceLabel||'the next document'} and return here later. This charter will remain incomplete.</p><footer><button type="button" className="btn-secondary" onClick={()=>setProgressionWarning(false)}>Stay here</button><button type="button" className="btn-primary" onClick={async()=>{setProgressionWarning(false);await continueToNextDocument()}}>Continue anyway</button></footer></section></div>}
  </div></WorkspaceShell>;
}
