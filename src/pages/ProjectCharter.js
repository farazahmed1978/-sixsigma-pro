...
export default function ProjectCharter() {
  ...
  const continueToSipoc=async()=>{if(!await saveNow())return;await updateProject(id,{resumeTarget:artifactResume({projectId:id,artifactId:'sipoc',artifactName:'SIPOC',sectionId:project.documents?.['document-sipoc']?.sectionState?.activeSectionId||''})});navigate(documentRoute(id,'sipoc'))};
