...
export default function DOEPage() {
  ...
  const rsmPlacementDraft=matrix?.fullAnalysis?(()=>{const analysis={toolId:'doe-rsm',toolType:'response-surface-methodology',title:`Response Surface Study — ${response}`,phase:'Improve',projectId:analysisContext?.projectId||activeDataset?.projectId||'',datasetIds:activeDataset?[activeDataset.id]:[],datasetVersionIds:activeDataset?.versionId?[activeDataset.versionId]:[],inputConfiguration:{designType:matrix.doeModel.designType,factors:matrix.doeModel.factors,alpha:matrix.doeModel.alpha,centerPoints:matrix.doeModel.centerPoints,blocks:matrix.doeModel.blocks,replicates:matrix.doeModel.replicates,randomSeed:matrix.doeModel.randomSeed},result:{design:matrix.doeModel,model:matrix.fullAnalysis,contourConfiguration:contour?{xFactor:contour.xFactor,yFactor:contour.yFactor,hold:contour.hold}:null,optimization,confirmationRuns:matrix.doeModel.confirmationRuns},interpretation:`A second-order response-surface model was fitted for ${response}. Review curvature, interactions, residual diagnostics, lack-of-fit limitations, and confirmation evidence before adopting settings.`,validationStatus:'UNVALIDATED'};return{toolId:'doe-rsm',projectId:analysis.projectId,title:analysis.title,analysis,reportItem:{...analysis,assetType:'analysis',statsSummary:{'R²':matrix.fullAnalysis.r2,'Adjusted R²':matrix.fullAnalysis.adjustedR2,'Model F':matrix.fullAnalysis.F,'Model p':matrix.fullAnalysis.pF},rawData:matrix.fullAnalysis.coefficients,provenance:{method:matrix.fullAnalysis.method,methodVersion:matrix.fullAnalysis.methodVersion,designType:matrix.doeModel.designType,randomSeed:matrix.doeModel.randomSeed,runOrder:matrix.doeModel.runs.map(run=>run.standardOrder),validationStatus:'UNVALIDATED'}}}})():null;
  const [savedArtifactId,setSavedArtifactId]=useState('');
  const saved=Boolean(savedArtifactId),saveRsm=async()=>{const result=await placement.requestPlacement({...rsmPlacementDraft,artifactId:savedArtifactId});if(result)setSavedArtifactId(result.artifactId)};
